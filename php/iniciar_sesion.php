...
/*$tipo_usuario = limpiar_cadena($_POST['login_tipo_usuario']);*/
    ...
    if($check_user->rowCount()==1){
    	...
    	if($check_user['usuario_usuario']==$usuario && password_verify($clave, $check_user['usuario_clave'])){
            ...
            /*switch ($tipo_usuario) {
                case 'administrador':
                    if(headers_sent()){
                        echo "<script> window.location.href='index.php?vista=homeusuario'; </script>";
                    }else{
                        header("Location: index.php?vista=homeusuario");
                    }
                    exit();
                case 'mantenimiento':
                    if(headers_sent()){
                        ...
                        header("Location: index.php?vista=category_new");
                    }
                    exit();
                case 'usuario':
                default:
